...
class LCMUpgradeApi(object):
    ...
    def upgrade_v5(self, body, **kwargs):  # noqa: E501
        ...
        Perform an upgrade for the VxRail software and hardware. Version 5 of this API includes the following options -- (1) \"missing_file_check\" - if set to true it will validate all files in the upgrade bundle are present. If set to false the upgrade bundle will not be checked. This is useful if you wish to create your own upgrade bundle i.e. Smart Bundle. Default value is set to true. (2)\"skip_failed_hosts\" - if set to true, any nodes that fail the upgrade will be ignored. Default value is set to false. (3) \"ecosystem_check\" - if set to true will ignore the ecosystem pre-check according to the inputs in \"continue_with_incompatible\" and \"components\". Default value is set to false.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
